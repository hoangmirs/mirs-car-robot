...
static esp_err_t index_handler(httpd_req_t *req)
{
  ...
  page += "<style>p { user-select: none; -webkit-user-select: none; -moz-user-select: none; -ms-user-select: none; }</style>\n";
  page += "<script>var xhttp = new XMLHttpRequest();</script>";
  page += "<script>function getsend(arg) { xhttp.open('GET', arg +'?' + new Date().getTime(), true); xhttp.send() } </script>";
  page += "<p align=center><IMG SRC='http://" + WiFiAddr + ":81/stream' style='width:360px;transform:rotate(0deg);'></p><br/><br/>";
